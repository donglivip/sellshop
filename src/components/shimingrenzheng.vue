<template>
	<div class="wrapper">
		<div class="header">
			...
			<div class="header-text">法人认证</div>
			<div class="header-aside"></div>
		</div>
		<div class="main">
			<div class="main-box">
				<div class="main-one">
					<div class="one-text">法人名字</div>
					<input type="text" placeholder="输入商户名字" class="name" v-model="name" />
				</div>
				<div class="main-one">
					<div class="one-text">法人手机</div>
					<input type="text" placeholder="输入商户电话" class="name" v-model="phone" />
				</div>
				<div class="main-two">
				...
				<div class="main-one">
					<div class="one-text">法人身份证号</div>
					<input type="number" placeholder="输入身份证号" class="name" v-model="shenfen" />
				</div>
				<div class="main-one">
					<div class="one-text">公司名字</div>
					<input type="text" placeholder="输入公司名称" class="name" v-model="mingcehng" />
				</div>
				<div class="main-one" @click="dizhichange()">
					<div class="one-text">公司所在地</div>
					<input type="text" placeholder="点击选择公司地址" class="name" v-model="qu" readonly=""/>
				</div>
				<div class="main-one">
					<div class="one-text">公司详细地址</div>
					<input type="text" placeholder="请输入公司详细地址" class="name" v-model="xiangxi"/>
				</div>
				<div class="main-three">上传身份证</div>
				<div class="main-four">
					<img src="../../static/shenfen.png" @click="upload('zimg')" v-if="zimg==''"/>
					<img :src="myurl03 + zimg" @click="upload('zimg')" v-if="zimg!=''"/>
					<div class="four-text">身份证正面</div>
				</div>
				<div class="main-four">
					<img src="../../static/shenfenfan.png" @click="upload('fimg')"  v-if="fimg==''"/>
					<img :src="myurl03 + fimg" @click="upload('fimg')" v-if="fimg!=''"/>
					<div class="four-text">身份证反面</div>
				</div>
				<div class="main-four">
					<img src="../../static/shenfen.png" @click="upload('simg')"  v-if="simg==''"/>
					<img :src="myurl03 + simg" @click="upload('simg')" v-if="simg!=''"/>
					<div class="four-text">手持身份证</div>
				</div>
<!--        <div class="main-four">
        	<img src="../../static/shenfen.png" @click="upload('wimg')"  v-if="wimg==''"/>
        	<img :src="myurl03 + simg" @click="upload('wimg')" v-if="wimg!=''"/>
        	<div class="four-text">卫生许可证</div>
        </div> -->
			</div>
			<div class="alert" v-if="dizhi">
		...
		myajax: function() {
			...
			$.ajax({
				...
				data: {
					...
					usTrName: that.name,
					usTrPhone: that.phone,
					usTrSex: that.index,
					usTrCompanyName: that.mingcehng,
					usTrCardId: that.shenfen,
					usTrCardFrontImgUrl: that.zimg,
					usTrCardBackImgUrl: that.fimg,
					usTrCardHandheldImgUrl: that.simg,
					usTrProvince :that.sheng,
					usTrCity :that.shi,
					usTrArea :that.qu,
					usTrDetailed :that.xiangxi
				},
				success: function(res) {
					if (res.data == 1) {
						plus.nativeUI.toast('已为您提交认证，等待后台审核中！');
						that.back();
					} else if(res.data==-1){
						alert('该地区暂无代理商，无法认证！');
					}else{
...
<style scoped>
	.alert{position: fixed;top: 0;left: 0;width: 100%;height: 100%;background: rgba(0,0,0,.5);}
.wrapper {
	background: #f7f7f9;
}
.header {
