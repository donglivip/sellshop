	...
	export default {
	  ...
	  methods:{
	  	onSelected:function(data){
				this.$store.state.province = data.province.value
				this.$store.state.city = data.city.value
				this.$store.state.area = data.area.value
			},
			citychange:function(type){
				if(type==0){
					if(this.$store.state.province=='选择现在住址'){
						this.$store.state.alerttext='地址选择不完整'
						this.$emit('alertchange')
					}
					this.$emit('citychange')
				}else{
					this.$store.state.province='选择现在住址'
					this.$emit('citychange')
				}
			}
	  },
